...
- 📡 **header centering**: a group in Appearance with two independent halves — center screen titles everywhere, and/or center chat/channel headers (text centers, avatar stays left); chats can be centered on their own, without the rest of the app. Under the chat half: switch the pill to compact mode, move the avatar to the right end of the pill, or keep the avatar out of the centering entirely (menu slot, or left where it normally sits). Every toggle off leaves the header full-width and left-aligned in both the island and classic UI
