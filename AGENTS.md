...
| `onMessagesControllerCreated(MessagesController, Int)` | `MessagesController.<init>` | per-account setup (maps provider; loads `PinHelper`'s local-pins cache and `PresenceHelper`'s watch-list cache; registers the `didReceiveNewMessages` → `onNewMessage` and `didUpdateConnectionState` → `GhostHelper.syncPresence` observers; runs `SavedMessagesHelper`/`PresenceHelper` TTL pruning) |
