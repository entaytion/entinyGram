...
function buildPrompt(info: BuildInfo, commits: Commit[], registry: RegistryEntry[]): string {
  ...
  const lines = [
    ...
    'A Telegram version bump is one bullet that says who did it: a commit like "rebase to X.Y.Z"',
    '(our own port onto stock Telegram) -> "Updated to Telegram X.Y.Z (ported by entinyGram)";',
    'an inugram sync that brings the new base -> "Updated to Telegram X.Y.Z (via inugram)".',
    '',
