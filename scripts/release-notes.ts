...
function buildPrompt(info: BuildInfo, commits: Commit[], registry: RegistryEntry[]): string {
  ...
  const lines = [
    ...
    '- In "tg_uk" only, wrap a flagship feature\'s name in **bold** (e.g. "[+] **Стрічка каналів**: ...").',
    '  Bold is for standalone flagship lines only - never bold inside a grouped "Added:"/"Fixed:" bullet,',
    '  and never use bold in "tg_en".',
  ...
  if (registry.length > 0) {
    ...
    if (relevant.length > 0) {
      ...
      lines.push(
        ...
        'of the entries below, wrap that feature name as a Markdown link. Never link inside',
        '"[*] Fixed" or "[-] Removed" bullets — links are only for new capabilities the reader',
        'might want to go turn on. Plain text everywhere else, including all of "tg_en" (EN skips',
