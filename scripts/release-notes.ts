...
function buildPrompt(info: BuildInfo, commits: Commit[], registry: RegistryEntry[]): string {
  ...
  const lines = [
    ...
    '- In "tg_uk" only, a flagship feature ALWAYS gets its own bullet like this:',
    '  "[+] **<name>:** [дієслово](tg://entinySettings/<slug>) ...rest..." — the feature NAME is plain',
    '  bold (no link on it), and the ACTION VERB right after the colon ("додано", "додає", etc.)',
    '  carries the deep link. E.g. "[+] **Pill Stack:** [додано](tg://entinySettings/pill-stack)',
    '  настроювані індикатори...". Never bold inside grouped "Added:"/"Fixed:" bullets,',
    '  never use bold in "tg_en".',
  ...
  if (registry.length > 0) {
    ...
    if (relevant.length > 0) {
      ...
      lines.push(
        ...
        'When a "[+]" bullet in "tg_uk" mentions a feature whose label closely matches one',
        'of the entries below, link the ACTION VERB (not the name): **[name]:** [дієслово](tg://entinySettings/<slug>).',
        'E.g. "[+] **Pill Stack:** [додано](tg://entinySettings/pill-stack) настроювані індикатори..." —',
        '"Pill Stack" is plain bold, "додано" is the link. Never link inside "[*] Fixed" or "[-] Removed"',
        'bullets — links are only for new capabilities the reader might want to go turn on.',
        'Plain text everywhere else, including all of "tg_en" (EN skips',
        'links entirely so it has room to stay readable within the same character budget).',
